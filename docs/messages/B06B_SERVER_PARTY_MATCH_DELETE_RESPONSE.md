# SERVER_PARTY_MATCH_DELETE_RESPONSE

> Previously documented as `CLIENT_ENTITY_SPAWN_CONFIRM` (client-derived).
...
| Handler(s) | `0x008840D0` |

### Fields

| # | Name | Type | Size | Description |
|---|------|------|------|-------------|
| 1 | `wErrorCode` | `bool` | 1 | if(packet.ReadBool() |
...
| String | Type |
|--------|------|
| `UIIT_MSG_PARTYMATCH_MODIFY_COMPLETE` | UI |
...
=== "C#"
    ```csharp
    // C#
    public record EntitySpawnConfirm(
        byte byResult,
        uint wErrorCode,
        uint dwField_03,
        byte byField_04,
...
=== "Go"
    ```go
    // Go
    type EntitySpawnConfirm struct {
        byResult uint8
        wErrorCode uint32
        dwField_03 uint32
        byField_04 uint8
...
=== "TypeScript"
    ```typescript
    // TypeScript
    export interface EntitySpawnConfirm {
        byResult: number;
        wErrorCode: number;
        dwField_03: number;
        byField_04: number;
